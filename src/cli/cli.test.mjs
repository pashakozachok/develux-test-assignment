...
import { validateNpmVersion, validateConfigFileExists, validatePackageExist } from './index.mjs';
...
describe('validateNpmVersion', () => {
  ...
  it('should throw an error for an invalid npm version', () => {
    expect(() => validateNpmVersion('invalid-version')).toThrowError('Error: Invalid npm version. Please provide a valid semantic version (X.Y.Z).');
  });
});
...
describe('validateConfigFileExists', () => {
  ...
  it('should throw an error if the config file does not exist', () => {
    ...
    expect(() => validateConfigFileExists('nonexistent-config.json')).toThrowError('Error: Invalid config file path.');
  });
});
...
describe('validatePackageExist', () => {
  ...
  it('should throw an error if the package does not exist on npm.io', async () => {
    ...
    try {
      ...
      expect(e.message).toBe('Error: Check if the nonexistent-package@1.0.0 exists on npm.io');
    }
  });
});